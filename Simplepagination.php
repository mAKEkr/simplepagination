			...
			if(is_array($configure) && ($configure['last_page'] >= $configure['current_page'])) {
				...
				if(($page != 0) && ($configure['last_page'] > 10)){
					if(($configure['last_page'] > 20) && ($configure['current_page'] > 20)) {
						$html_return_string .= '<li><a href="' . site_url($configure['url'] . '/1') . '" class="firstpage">&laquo;</a></li>';
					}
					$html_return_string .= '<li><a href="' . site_url($configure['url'] . '/' . ($page-9) ) . '" class="prevpage">&lt;</a></li>';
				}
				...
				for($i=$page+1; $i<=$page+$loop_page_count; $i++){
					if($i == $configure['current_page']){
						...
						$html_return_string .= '<li><a href="' . site_url($configure['url'] . '/' . $i) . '">' . $i . '</a></li>';
					}
				}

				if(($configure['last_page'] > 10) && ( $page+10 < $configure['last_page'])) {
					$html_return_string .= '<li><a href="' . site_url($configure['url'] . '/' . ($page+11)) . '" class="nextpage">&gt;</a></li>';
					if(($configure['last_page'] - $page) > 20){
						if(is_int($configure['last_page'] / 10)) {
							$html_return_string .= '<li><a href="' . site_url($configure['url'] . '/' . (((int)($configure['last_page'] / 10))*10)) . '" class="lastpage">&raquo;</a></li>';
						} else {
							$html_return_string .= '<li><a href="' . site_url($configure['url'] . '/' . (((int)($configure['last_page'] / 10))*10 + 1)) . '" class="lastpage">&raquo;</a></li>';
						}
					}
				}
